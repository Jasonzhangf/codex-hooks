...
if (operation === "operator-enable" || operation === "operator-disable") {
  ...
  await mutate({ operation: "schedule.upsert", id: required(args[0], "schedule id"), at: required(args[1], "schedule time"), body: required(args[2], "schedule body"), target: parseTarget(args[3]) });
} else if (operation === "schedule-remove") {
  await mutate({ operation: "schedule.remove", id: required(args[0], "schedule id") });
} else {
  throw new Error("usage: cli.js operator-enable|operator-disable <name> | schedule-upsert <id> <at> <body> <target-json> | schedule-remove <id>");
}
